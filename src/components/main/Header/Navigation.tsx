...
const menuColumns = [
  {
    label: '전마협',
    items: [
    { name: '협회소개', href: '/association/introduce' },
  ...
  },
  {
    label: '쇼핑몰',
    items: [
      { name: '쇼핑몰', href: '/shop/merchandise' },
    ],
  },
  {
    label: '마이페이지',
...
export default function Navigation() {
  ...
  return (
    <>
      <nav className="hidden md:grid items-center border-x border-orange-400" style={{ gridTemplateColumns: 'repeat(6, minmax(120px, 1fr))' }}>
        {menuColumns.map((col) => (
          <div key={col.label} className="flex items-center justify-center h-16 border-x border-orange-400">
            <button
              onClick={() => setActiveMenu(activeMenu === col.label ? null : col.label)}
              className="w-full h-full flex items-center justify-center text-center font-pretendard text-gray-900 hover:text-blue-600 transition-colors whitespace-nowrap break-keep truncate"
            >
              {col.label}
            </button>
          </div>
            ...
            <div className="w-full max-w-[1920px] mx-auto px-4">
              ...
              <div className="flex justify-center py-12">
                ...
                <div className="grid" style={{ gridTemplateColumns: 'repeat(6, minmax(120px, 1fr))' }}>
                  {menuColumns.map((col) => (
                    <div key={col.label} className="flex flex-col items-center overflow-hidden border-x border-orange-400">
      ...
      {activeMenu === 'mobile' && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute top-16 left-0 right-0 bg-white shadow-lg border-t border-orange-400">
            <div className="py-4">
              {menuColumns.map((col) => (
                <div key={col.label} className="border-b border-gray-200 last:border-b-0">
                  <button
                    onClick={() => setActiveMenu(activeMenu === col.label ? null : col.label)}
                    className="w-full px-4 py-3 text-left font-pretendard text-gray-900 hover:bg-gray-50 transition-colors"
                  >
                    {col.label}
                  </button>
                    ...
                    <div className="bg-gray-50 px-4 py-2">
                      {col.items.map((subItem) => (
                        <a
                          key={subItem.name}
                          href={subItem.href}
                          className="block py-2 text-sm font-pretendard text-gray-600 hover:text-blue-600 transition-colors"
                          onClick={() => setActiveMenu(null)}
                        >
                          {subItem.name}
                        </a>
