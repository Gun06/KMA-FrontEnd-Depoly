...
export default function PageMediaList({
  ...
  return (
    <>
      <ul
        className={cn(
          "grid h-40 grid-rows-1 overflow-hidden rounded border bg-black/40",
          gridCols
        )}
      >
        {mediaItems.slice(0, visibleCount).map((item, index) => {
          ...
          return (
            <li key={`${item.url}-${index}`} className="relative min-w-0 h-full overflow-hidden">
              <button
                type="button"
                onClick={() => setPreviewIndex(index)}
                className="group relative block h-full w-full cursor-pointer hover:opacity-80 transition-opacity"
                aria-label={label}
              >
                <img
