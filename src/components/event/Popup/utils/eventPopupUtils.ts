import { EventPopupApiResponse, EventPopupItem, DeviceType } from '../types';
...
/**
 * API 응답을 EventPopupItem 형식으로 변환
 */
export const transformEventApiResponseToPopupItem = (apiResponse: EventPopupApiResponse[]): EventPopupItem[] => {
...
export const shouldShowToday = (): boolean => {
  try {
    ...
    const stored = localStorage.getItem('kma_event_popup_dont_show_today');
    return stored !== today;
  } catch {
    return true;
  }
...
export const setDontShowToday = (): void => {
  try {
    const today = new Date().toDateString();
    localStorage.setItem('kma_event_popup_dont_show_today', today);
  } catch {
    // ignore
  }
};
