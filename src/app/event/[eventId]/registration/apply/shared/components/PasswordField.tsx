...
export default function PasswordField({ 
  ...
  minLength = 6,
  helperText = "비밀번호는 최소 6자리, 공백 없이 입력해주세요.",
  showValidationGuide = true
}: PasswordFieldProps) {
  const isValid = (value || '').length >= minLength && !/\s/.test(value || '');
      ...
      {showValidationGuide && value && (
        <div className="mt-2 space-y-1">
          <div className={`flex items-center text-xs ${lengthOk ? 'text-green-600' : 'text-red-500'}`}>
            ...
            길이: 최소 {minLength}자리 ({value.length}자)
          </div>
