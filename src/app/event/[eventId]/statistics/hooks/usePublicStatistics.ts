...
/**
 * 대회별 종목 통계 데이터 조회 Hook (공개용)
 * @param eventId - 대회 고유 번호
 * @returns 종목 통계 데이터 및 로딩/에러 상태
 */
export function usePublicEventDistanceStatistics(eventId: string) {
  return useQuery({
    queryKey: ['public', 'events', 'distance-statistics', eventId],
    queryFn: async () => {
      if (!eventId) {
        throw new Error('Event ID is required');
      }

      const url = `${API_BASE_URL_ADMIN}/api/v1/${eventId}/registration/distancd-statistics`;
