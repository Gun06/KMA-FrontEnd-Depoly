...
type UseLocalEventDetailOptions = {
  /** false면 쿼리 비활성화 (예: 메인 유저 상세에서 관리자 GET을 막을 때) */
  enabled?: boolean;
};
 ...
 */
export function useLocalEventDetail(
  eventId: string,
  options?: UseLocalEventDetailOptions
) {
  const idOk = !!eventId && eventId.trim() !== '';
