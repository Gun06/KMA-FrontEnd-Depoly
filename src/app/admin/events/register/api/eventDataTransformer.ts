// src/app/admin/events/register/api/eventDataTransformer.ts
...
export class EventDataTransformer {
  ...
  static transformToServerFormat(frontendData: EventCreatePayload): {
    ...
    // 4. 최종 요청 데이터 구성 (기념품/종목은 별도 API로 전송)
  ...
  static validateImages(imageFiles: EventImageFiles): {
    ...
    // 이미지 키를 사용자 친화적인 이름으로 매핑
    const imageNameMap: Record<string, string> = {
      'mainBannerPcImage': '메인 배너 (PC)',
    ...
    for (const imageKey of requiredImages) {
      ...
      if (!file || file.size === 0 || file.name === 'default.png') {
        const displayName = imageNameMap[imageKey] || imageKey;
        errors.push(
          `❌ ${displayName}이(가) 필요합니다. (현재: ${file?.name || '없음'})`
        );
      }
    }
