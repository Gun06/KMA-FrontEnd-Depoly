...
import { getMainFaqDetail, updateMainFaq } from "@/data/faq/main";
...
export default function Page() {
  ...
  useEffect(() => {
    const it = getMainFaqDetail(Number(faqId));
    if (it) {
      setTitle(it.title || "");
      setQuestionContent(it.question || "");
      setAnswerContent(it.answer?.content || "");
      setAnswerFiles(it.answer?.files ?? []);
    }
  }, [faqId]);
      ...
      updateMainFaq(
        Number(faqId),
        {
          ...
          answer: answerContent
            ? { content: answerContent, files: answerFiles }
            : null,
        },
        { adminName: "관리자" }
      );
