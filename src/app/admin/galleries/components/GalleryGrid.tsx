...
export default function GalleryGrid({ items, onItemClick }: GalleryGridProps) {
  ...
  return (
    <div className="w-full py-6">
      <ul className="flex flex-wrap items-start gap-3 md:gap-6">
        {items.map((item) => (
          <li key={item.id} className="shrink-0">
            <button
              type="button"
              onClick={onItemClick ? () => onItemClick(item.id) : undefined}
              className="focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-2xl transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg"
            >
              <GalleryCard
