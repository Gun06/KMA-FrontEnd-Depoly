...
export default function SchedulePage() {
  ...
  const scheduleQueries = useQueries({
    queries: Array.from({ length: 12 }, (_, i) => ({
      queryKey: ['schedule', year, i + 1, apiType, scheduleBlockFilter],
      queryFn: () => fetchScheduleEvents(year, i + 1, apiType, scheduleBlockFilter),
      staleTime: 1000 * 60 * 5,
      gcTime: 1000 * 60 * 10,
    })),
  });
        ...
        >
          ...
          <div className="px-2 pt-2 pb-2">
            <div className="overflow-x-auto no-scrollbar">
              <div className="flex w-full min-w-[252px] gap-1 rounded-lg bg-gray-100 p-1">
              <button
                onClick={() => handleViewModeChange('marathon')}
                className={clsx(
                  'flex-1 py-2 px-2 text-xs font-medium rounded-md transition-colors whitespace-nowrap',
                  viewMode === 'marathon'
                              ...
                              <ul className="grid grid-cols-2 sm:grid-cols-3 min-[915px]:grid-cols-4 min-[1260px]:grid-cols-5 min-[1560px]:grid-cols-6 gap-4 md:gap-5 lg:gap-6 auto-rows-fr list-none">
                                {filteredMonthEvents.map(event => {
                                  ...
                                  const eventDateStr = typeof event.eventDate === 'string' && event.eventDate.includes('T')
                                    ...
                                    : event.eventDate;
                                  const rawDeadline =
                                    (event.registDeadline &&
                                      typeof event.registDeadline === 'string' &&
                                      event.registDeadline.trim())
                                      ? event.registDeadline
                                      : event.eventDeadLine;
                                  const eventDeadLineStr =
                                    rawDeadline &&
